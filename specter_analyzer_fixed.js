...
try {
  ...
  if (supabaseUrl && supabaseKey) {
    ...
    console.log("⚠️  Supabase environment variables not found - analysis will run without database save");
  }
} catch (error) {
  console.log("⚠️  Could not initialize Supabase:", error.message);
}
...
async function analyzeRepository(owner, repoName) {
  ...
  try {
    ...
    // 5. Save to database if Supabase is available
    if (supabase) {
      try {
        ...
        if (error) {
          ...
          console.log(`   ✅ ${repoName.toUpperCase()}: ${totalLOC.toLocaleString()} LOC, ${primaryLang} (Saved to DB)`);
...
async function main() {
  ...
  if (supabase) {
    ...
    console.log("\n✅ Analysis complete. Run with proper environment variables to save to database.");
  }
}
